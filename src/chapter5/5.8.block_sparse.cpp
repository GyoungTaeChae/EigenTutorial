...
int main() {
    std::cout << "=== 5.8 Block Sparse Operations ===\n\n";
    ...
    Eigen::Matrix3d block_00 = Eigen::Matrix3d::Identity() * 10;
    Eigen::Matrix3d block_11 = Eigen::Matrix3d::Identity() * 10;
    Eigen::Matrix3d block_01 = Eigen::Matrix3d::Ones() * (-1);

    // Assemble into sparse
    typedef Eigen::Triplet<double> T;
    std::vector<T> block_trips;
    for (int i = 0; i < 3; ++i) {
        for (int j = 0; j < 3; ++j) {
            if (std::abs(block_00(i, j)) > 1e-10)
                block_trips.push_back(T(i, j, block_00(i, j)));
            if (std::abs(block_11(i, j)) > 1e-10)
                block_trips.push_back(T(3 + i, 3 + j, block_11(i, j)));
            if (std::abs(block_01(i, j)) > 1e-10) {
    ...
    Eigen::SparseMatrix<double> block_sparse(6, 6);
    block_sparse.setFromTriplets(block_trips.begin(), block_trips.end());
    std::cout << "Block sparse matrix:\n" << Eigen::MatrixXd(block_sparse) << "\n";
