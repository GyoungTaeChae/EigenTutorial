...
int main() {
    cout << "=== 6.5 Levenberg-Marquardt Method ===\n\n";
    ...
    vector<double> data_x = {0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0};
    ...
    cout << "Initial guess: a = " << a_gn << ", b = " << b_gn << "\n\n";
    ...
    for (int iter = 0; iter < 20; ++iter) {
        ...
        // LM update
        Matrix2d H = J.transpose() * J;
        Vector2d g = -J.transpose() * r;

        // Add damping
        Matrix2d H_damped = H + lambda * Matrix2d::Identity();
        Vector2d dx = H_damped.ldlt().solve(g);
        ...
        if (new_cost < cost) {
            ...
            b_gn = b_new;
            lambda /= 2;
            cout << "Iter " << iter << ": ACCEPT, a = " << a_gn << ", b = " << b_gn
            ...
            lambda *= 2;
            cout << "Iter " << iter << ": REJECT, increasing lambda to " << lambda << "\n";
        }
    }
